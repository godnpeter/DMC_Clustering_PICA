...
class MLPModel(nn.Module):
    ...
    def __init__(self, num_inputs, output_size, recurrent=True):
    ...
    def forward(self, inputs, hidden, masks):
        ...
        batch_size = op.size(0)
        rand = torch.rand(batch_size, 5)#.cuda()
        ...
        if self.resultprint == True:
            if self.action_idx != hp.argmax() :
                be = self.action_idx
                af = hp.argmax()
                self.action_idx = hp.argmax()
