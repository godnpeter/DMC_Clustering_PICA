...
class ProjectionLayer(nn.Module):
    def __init__(self, num_inputs, output_size):
        ...
        init_ = init_easy(gain=np.sqrt(2), bias=0)
        self.linear1 = init_(nn.Linear(num_inputs, num_inputs))
        self.ReLU = nn.ReLU()
        self.linear2 = init_(nn.Linear(num_inputs, output_size))
...
class MLPBlock_ReLU(nn.Module):
    def __init__(self, num_inputs, output_size):
        super(MLPBlock_ReLU, self).__init__()
        init_ = init_easy(gain=np.sqrt(2), bias=0)
        self.linear1 = init_(nn.Linear(num_inputs, output_size))
        self.ReLU1 = nn.ReLU()
        self.linear2 = init_(nn.Linear(output_size, output_size))
    ...
    def forward(self, inputs):
        x = self.ReLU1(self.linear1(inputs))
